...
#onlineicon = pygame.transform.scale(onlineicon, (120,90))
...
englishbutton = pygame.transform.scale(englishbutton, (120,120))
danishbutton = pygame.transform.scale(danishbutton, (120,120))
...
if __name__ == '__main__':        
    ...
    tracking_thread = mp.Process(target=faceTracking, args=(sender,))
    ...
    while not done:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if show_buttons:
                    if event.x < 0.12 and event.y < 0.12:  # values differ for only touchscreen
                        print("Left button pressed")
                        interrupted = True
                        no_detected = True
                    if event.x > 0.88 and event.y < 0.12:
                        print("Right button pressed")
                        interrupted = True
                        yes_detected = True
                if state == MENUSTATE:
                    if 160 < event.x*800 < 100 and 0.27 < event.y < 0.59:
                        state = NORMALSTATE
                        if LANGUAGE == "en-US":
                            threadevent.set()
                            LANGUAGE = "da-DK"
                    if 0.60 < event.x < 0.85 and 0.27 < event.y < 0.59:
                        state = NORMALSTATE
                        if LANGUAGE == "da-DK":
                            threadevent.set()
                            LANGUAGE = "en-US"
                    if event.x > 0.94 and event.y > 0.88:
                        state = COLORSTATE
        ...
        if state == NORMALSTATE:
            ...
            screen.blit(englishbutton, (800-160-120, 100))
            screen.blit(onlineicon, (160, 250))
            screen.blit(coloricon, (800-160-120, 250))      
            #screen.blit(offlineicon, (800-120-200, 200)) 
