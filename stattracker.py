...
class StatTracker:
    def __init__(self):
        ...
        self.labels = ["Activations", "People"]
    ...
    def list_with_operating_hours(self):
        ...
        for i in range(length):
            ...
            self.peopleListPlot.append(0)
        return hoursList
    
    def update_plot(self, numberOfActivations, numberOfPeople):
        ...
        if hourString in self.hoursList:
            ...
            self.numberOfPeopleHour = numberOfPeople
        if hour != self.lastHour and self.operatingTime == True:
            ...
            trailingPeople = numberOfPeople - self.numberOfPeopleHour
            self.peopleListPlot.insert(indexHour, round(trailingPeople))
            self.peopleListPlot.pop(indexHour + 1)
            self.numberOfPeopleHour = numberOfPeople
            #print(trailingActivations)
        self.lastHour = hour
    ...
    def get_plot(self):
        plt.clf()
        plt.bar(self.xlist-0.2, self.activationsList, width=0.3)
        plt.bar(self.xlist+0.2, self.peopleListPlot, width=0.3)
        plt.title('Number of Activations and People')
