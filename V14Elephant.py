...
def faceTracking(listproxy, peoplecounter, terminator):
    ...
    while not terminator.value:
        ...
        faces = engine.detect_with_image(framePIL,
                                         ...
                                         top_k=10)
        for face in faces:
            #(x1, y1, x2, y2) = face.bounding_box.flatten().tolist()
...
def drivePupil():
    ...
    if noPeople:
        ...
        zeroPositionRight[0] = 90
        zeroPositionRight[1] = 90
        servo.move_servo_position(0, zeroPositionLeft[0])
        servo.move_servo_position(1, zeroPositionLeft[1])
        servo.move_servo_position(2, zeroPositionRight[0])
        servo.move_servo_position(3, zeroPositionRight[1])
...
ELEPHANTEYESEVENT = pygame.USEREVENT + 14
PUMPINGEVENT = pygame.USEREVENT + 15
...
if __name__ == '__main__':        
    ...
    pygame.time.set_timer(ELEPHANTEYESEVENT, 1000)
    pumpindex = 0
    pumpangles = [180,0,180]
    activated = False
    ...
    while not done:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if state == NORMALSTATE: state = MONSTERSTATE
                elif state == MONSTERSTATE: state = NORMALSTATE
            ...
            if event.type == INTERACTIONEVENT:
                ...
                set_angle(servo, pumpangles[pumpindex])
                if pumpindex == 2:
                    pumpindex = 0
                else:
                    pumpindex += 1
                    pygame.time.set_timer(PUMPINGEVENT, 500)
        ########## Interaction ##########
        ...
        waitTimer = 0
        if listproxy[0]:
            trackedList = listproxy[0]
            peopleCount = peoplecounter.value
        ...
        if mcp.read_adc(0) > 100 and activated == False:
            numberOfActivations += 1
            activated = True
            pygame.event.post(pumpingevent)
        elif activated == True and sensorValue < 100:
            activated = False       
